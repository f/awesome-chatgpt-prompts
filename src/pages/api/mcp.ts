...
import { mcpRequestContext, getMcpContext, type AuthenticatedUser, type McpRequestContext } from "@/lib/mcp/context";
...
// Build category/tag filter for prompts using AsyncLocalStorage context
// If authenticated user is present and no specific users filter, include their private prompts
function buildPromptFilter(includeOwnPrivate: boolean = true): Record<string, unknown> {
  ...
  if (authenticatedUser && includeOwnPrivate) {
    ...
    const usersFilter = ctx.users && ctx.users.length > 0 ? ctx.users : null;
...
// --- Singleton McpServer ---
// Created once at module level. Tool handlers read per-request auth via AsyncLocalStorage.
...
server.registerTool(
  ...
  async ({ query, limit = 10, type, category, tag }) => {
    const { authenticatedUser } = getMcpContext();
    try {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const ctx: McpRequestContext = {
    authenticatedUser,
    categories: categoriesParam ? categoriesParam.split(",").map((c) => c.trim()) : undefined,
    tags: tagsParam ? tagsParam.split(",").map((t) => t.trim()) : undefined,
    users: usersParam ? usersParam.split(",").map((u) => u.trim()) : undefined,
  };

  // Run handler in AsyncLocalStorage context — each request gets its own isolated context
  return mcpRequestContext.run(ctx, async () => {
    try {
      ...
      res.on("close", () => {
        transport.close();
        // DO NOT call server.close() — singleton must stay alive across requests
      });
    } catch (error) {
