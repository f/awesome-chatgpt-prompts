Cada gran prompt comparte elementos estructurales comunes. Entender estos componentes te permite construir prompts sistemáticamente en lugar de por prueba y error.

<Callout type="tip" title="Los Bloques de Construcción">
Piensa en estos componentes como piezas de LEGO. No necesitas todos para cada prompt, pero saber qué está disponible te ayuda a construir exactamente lo que necesitas.
</Callout>
...
<PromptBreakdown parts={[
  ...
  { label: "Tarea", text: "Revisa este código buscando errores" },
  ...
  { label: "Ejemplo", text: "Como: 1. Riesgo de inyección SQL en línea 42" }
]} />
...
<Compare 
  ...
  after={{ label: "Con Rol", content: "Eres un profesor de física especializado en hacer accesibles temas complejos para principiantes. Explica la computación cuántica." }}
/>
...
<Compare 
  ...
  after={{ label: "Contexto Fuerte", content: "Estoy construyendo una API REST en Node.js usando Express.js. La API maneja autenticación de usuarios con tokens JWT. Cuando un usuario intenta acceder a una ruta protegida, recibe un error 403 incluso con un token válido. Aquí está el código relevante: [código]" }}
/>
...
La tarea es el corazón de tu prompt—lo que quieres que el modelo haga. Sé específico e inequívoco.
...
<div className="my-4 grid gap-2">
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
    ...
    <span className="text-muted-foreground">Escribe, Crea, Genera, Compón, Diseña</span>
  </div>
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
  ...
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
    ...
    <span className="text-muted-foreground">Explica, Describe, Clarifica, Define, Ilustra</span>
  </div>
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
    <span className="font-semibold min-w-32">Resolución</span>
...
"Escribe como si hablaras a un niño de 10 años"
...
Especificar el formato de salida asegura que obtengas respuestas en una estructura usable.
...
**Datos estructurados:**
```
"Devuelve como JSON con claves: titulo, descripcion, prioridad"
...
Reseña: "El producto llegó rápido y funciona genial, pero 
...
<TryIt 
  ...
  prompt={`# Rol
Eres un redactor técnico senior con 10 años de experiencia creando documentación para desarrolladores.

# Contexto
Estoy documentando una API REST para un servicio de procesamiento de pagos. La audiencia son desarrolladores integrando nuestra API en sus aplicaciones. Tienen conocimiento intermedio de programación pero pueden ser nuevos en conceptos de procesamiento de pagos.
...
- Asume que los lectores entienden HTTP y JSON básico

# Formato de Salida
Estructura la documentación como:
1. Descripción del Endpoint (2-3 oraciones)
2. Request (método, URL, headers, body con ejemplo)
3. Response (ejemplos de éxito y error)
4. Ejemplo de Código (en JavaScript/Node.js)

# Detalles del Endpoint
POST /v1/payments/intents
Body: { "amount": 1000, "currency": "usd", "description": "Orden #1234" }`}
/>

## El Prompt Mínimo Efectivo

No todos los prompts necesitan todos los componentes. Para tareas simples, una instrucción clara puede ser suficiente:
...
## Patrones de Prompt Comunes

Estos frameworks te dan una lista de verificación simple a seguir al escribir prompts. Haz clic en cada paso para ver un ejemplo.
...
- Crear plantillas de prompt reutilizables
- Comunicar tus intenciones claramente

<Quiz 
  question="¿Qué componente tiene el mayor impacto en la calidad de respuesta?"
  options={[
...
¡Ahora es tu turno! Usa este constructor de prompts interactivo para crear tu propio prompt usando los componentes que has aprendido:
...
<PromptChallenge
  ...
  task="Escribe un prompt que pida a una IA revisar código por vulnerabilidades de seguridad. Tu prompt debe ser lo suficientemente específico para obtener retroalimentación accionable."
  criteria={[
    "Incluye un rol claro o nivel de expertise",
    "Especifica qué tipo de revisión de código (enfoque de seguridad)",
    "Define el formato de salida esperado",
    "Establece restricciones o alcance apropiados"
  ]}
  hints={[
    "Piensa en qué expertise debería tener un revisor de código",
    "Sé específico sobre qué problemas de seguridad buscar",
    "Considera pedir un formato de respuesta estructurado"
  ]}
  exampleSolution={`Eres un ingeniero de seguridad senior con expertise en seguridad de aplicaciones web y vulnerabilidades OWASP Top 10.

Tarea: Revisa el siguiente código por vulnerabilidades de seguridad.

Enfócate en:
- Riesgos de inyección SQL
- Vulnerabilidades XSS
- Problemas de autenticación/autorización
- Gaps de validación de entrada
