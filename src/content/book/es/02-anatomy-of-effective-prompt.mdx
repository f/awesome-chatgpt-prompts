Todo gran prompt comparte elementos estructurales comunes. Entender estos componentes te permite construir prompts sistemáticamente en lugar de por prueba y error.

<Callout type="tip" title="Los Bloques de Construcción">
Piensa en estos componentes como bloques LEGO. No necesitas todos ellos para cada prompt, pero saber qué está disponible te ayuda a construir exactamente lo que necesitas.
</Callout>
...
<Compare 
  ...
  after={{ label: "Con Rol", content: "Eres un profesor de física que se especializa en hacer accesibles temas complejos para principiantes. Explica la computación cuántica." }}
/>
...
<Compare 
  ...
  after={{ label: "Contexto Fuerte", content: "Estoy construyendo una API REST en Node.js usando Express.js. La API maneja autenticación de usuarios con tokens JWT. Cuando un usuario intenta acceder a una ruta protegida, obtiene un error 403 incluso con un token válido. Aquí está el código relevante: [código]" }}
/>
...
- **Objetivos** — Qué estás tratando de lograr en última instancia
- **Restricciones** — Límites de tiempo, requisitos técnicos, guías de estilo

## 3. Tarea / Instrucción

La tarea es el corazón de tu prompt—lo que quieres que haga el modelo. Sé específico e inequívoco.
...
<SpecificitySpectrum levels={[
  ...
  { level: "Óptimo", text: "Edita este ensayo para gramática y claridad, manteniendo el tono original pero reduciendo la palabrería en un 20%" }
]} />
...
"Sé directo y evita el lenguaje evasivo"
```
...
## 6. Ejemplos (Aprendizaje con Pocos Ejemplos)

Los ejemplos son la forma más poderosa de mostrarle al modelo exactamente lo que quieres.

### Ejemplo con Un Solo Caso

```
Convierte estas oraciones al tiempo pasado.
...
Clasifica: "La app se bloquea cuando abro configuración"
```
...
3. Por qué importa para equipos remotos`}
  description="Este prompt usa los seis componentes. Pruébalo y observa cómo el enfoque estructurado produce resultados enfocados y accionables."
/>
...
<PromptChallenge
  ...
  exampleSolution={`Eres un ingeniero de seguridad senior con experiencia en seguridad de aplicaciones web y vulnerabilidades del OWASP Top 10.
