La ingeniería de prompts rara vez es un proceso de un solo intento. Los mejores prompts emergen a través de la iteración—probando, observando y refinando hasta lograr los resultados deseados.

<Callout type="info" title="Primer Borrador, No Borrador Final">
Piensa en tu primer prompt como un borrador. Incluso los ingenieros de prompts experimentados rara vez lo logran al primer intento.
</Callout>

## El Ciclo de Iteración

El refinamiento efectivo de prompts sigue un ciclo predecible: escribir, probar, analizar y mejorar. Cada iteración te acerca más a un prompt que produce confiablemente los resultados que necesitas.

<IterativeRefinementDemo />

## Patrones Comunes de Refinamiento

La mayoría de las fallas en prompts caen en un puñado de categorías. Aprender a reconocer estos patrones te permite diagnosticar y arreglar problemas rápidamente sin empezar desde cero.
...
Los prompts vagos producen salidas vagas. El modelo no puede leer tu mente sobre qué significa "mejor" o qué aspectos te importan más.
...
El tono es subjetivo y varía según el contexto. Lo que suena "profesional" para el modelo puede no coincidir con la voz de tu organización o la relación con tu destinatario.

<DiffView 
  before="Escribe un email de disculpa por no cumplir una fecha límite."
  after={`Escribe un email de disculpa profesional pero cálido por no cumplir una fecha límite de proyecto. El tono debe ser responsable sin ser excesivamente apologético. Incluye un plan concreto para prevenir futuros retrasos.`}
  beforeLabel="Original"
  afterLabel="Refinado"
/>
...
Las solicitudes abiertas obtienen respuestas abiertas. Si necesitas tipos específicos de feedback, debes pedirlos explícitamente.
...
Para cada problema encontrado, explica el problema y sugiere una solución.
...
## Enfoque de Refinamiento Sistemático
...
Antes de cambiar algo, identifica qué está realmente mal. Usa esta tabla de diagnóstico para mapear síntomas a soluciones:
...
<div className="my-4 grid gap-2">
  ...
  <div className="grid grid-cols-3 gap-2 p-3 bg-red-50 dark:bg-red-950/20 rounded-lg text-sm">
    ...
    <span className="text-green-600 dark:text-green-400">Agregar ejemplos few-shot</span>
  </div>
</div>

### Paso 2: Hacer Cambios Dirigidos

Resiste la tentación de reescribir todo. Cambiar múltiples variables a la vez hace imposible saber qué ayudó y qué perjudicó. Haz un cambio, pruébalo, luego procede:
...
### Paso 3: Documentar Lo Que Funciona

El conocimiento de ingeniería de prompts se pierde fácilmente. Mantén un registro de lo que probaste y por qué. Esto ahorra tiempo cuando revisites el prompt más tarde o enfrentes desafíos similares:
...
Tono: Amigable, profesional, empático pero sin exagerar."
```

## Ejemplo de Iteración del Mundo Real

Recorramos un ciclo completo de iteración para ver cómo cada refinamiento se construye sobre el anterior. Nota cómo cada versión aborda deficiencias específicas de la anterior.

### Tarea: Generar Nombres de Productos
...
<VersionDiff versions={[
  ...
  {
    label: "Versión 2",
    content: `Genera nombres para una nueva app de productividad. La app usa IA para programar automáticamente tus tareas basándose en niveles de energía y disponibilidad del calendario.`,
...
- Evitar: palabras genéricas (pro, smart, AI, task, flow)
...
Diferentes tareas fallan de maneras predecibles. Conocer los modos de falla comunes te ayuda a diagnosticar y arreglar problemas más rápido.

### Para Generación de Contenido

La generación de contenido a menudo produce salidas genéricas, fuera de objetivo o mal formateadas. La solución usualmente implica ser más específico sobre restricciones, proporcionar ejemplos concretos o definir la voz de tu marca explícitamente.

<InfoGrid items={[
  { icon: "target", title: "Muy Genérico", description: "Agregar restricciones y contexto específicos", example: "\"Escribe sobre perros\" → \"Escribe sobre golden retrievers para dueños primerizos, enfocándote en entrenamiento y necesidades de ejercicio\"" },
  ...
  { icon: "megaphone", title: "Fuera de Marca", description: "Incluir guías de voz de marca", example: "\"Usa tono amigable y casual. Evita jerga. Dirige al lector como 'tú'.\"" }
]} />

### Para Generación de Código

La salida de código puede fallar técnicamente (errores de sintaxis, características incorrectas del lenguaje) o arquitectónicamente (patrones pobres, casos faltantes). Los problemas técnicos necesitan especificaciones de versión/entorno; los problemas arquitectónicos necesitan guía de diseño.
...
<InfoGrid items={[
  ...
  { icon: "shield", title: "Faltan Casos Límite", description: "Listar escenarios a manejar", example: "\"Manejar: entrada vacía, valores null, timeouts de red, formatos inválidos\"" },
  { icon: "tag", title: "Nombres Pobres", description: "Incluir convenciones de nombres", example: "\"Usa camelCase para variables, PascalCase para clases, UPPER_SNAKE para constantes\"" }
]} />

### Para Análisis

Las tareas de análisis a menudo producen resultados superficiales o sin estructura. Guía al modelo con frameworks específicos (FODA, Cinco Fuerzas de Porter), solicita múltiples puntos de vista o proporciona una plantilla para la estructura de salida.
...
<InfoGrid items={[
  ...
  { icon: "database", title: "Faltan Datos", description: "Especificar qué analizar", example: "\"Enfócate en: tamaño de mercado, tasa de crecimiento, actores clave, barreras de entrada\"" },
  { icon: "layout", title: "Sin Estructura", description: "Proporcionar plantilla de análisis", example: "\"Formato como: Resumen → Hallazgos Clave → Implicaciones → Recomendaciones\"" }
]} />

### Para Preguntas y Respuestas

Las respuestas a preguntas pueden ser muy breves o muy largas, y pueden carecer de indicadores de confianza o fuentes. Especifica el nivel de detalle que necesitas y si quieres citas o expresión de incertidumbre.
...
<InfoGrid items={[
  ...
  { icon: "link", title: "Sin Fuentes", description: "Solicitar citas", example: "\"Cita fuentes para las afirmaciones\" o \"Incluye referencias donde sea posible\"" }
]} />

## La Técnica del Bucle de Retroalimentación
...
Para prompts que se usarán repetidamente o a escala, no elijas simplemente el primero que funcione. Prueba variaciones para encontrar el enfoque más confiable y de mayor calidad.

```
Prompt A: "Resume este artículo en 3 viñetas."
Prompt B: "Extrae los 3 insights más importantes de este artículo."
...
La perfección es enemiga de lo suficientemente bueno. Sabe cuándo tu prompt está listo para usar y cuándo solo estás puliendo con rendimientos decrecientes.
...
<div className="my-6 grid md:grid-cols-2 gap-4">
  ...
  <div className="border rounded-lg bg-red-50/50 dark:bg-red-950/20 border-red-200 dark:border-red-900">
    <p className="text-sm font-semibold text-red-700 dark:text-red-400 px-4 pt-3 flex items-center gap-2 m-0!"><IconX className="h-4 w-4" /> Seguir Iterando</p>
    <div className="text-sm p-4 pt-2 space-y-1">
      <p className="m-0!">La salida es inconsistente entre ejecuciones</p>
      <p className="m-0!">Los casos límite causan fallas</p>
...
Los prompts son código. Para cualquier prompt usado en producción, trátalo con el mismo rigor: control de versiones, changelogs y la capacidad de revertir si algo se rompe.

<Callout type="tip" title="Versionado Integrado">
prompts.chat incluye historial de versiones automático para tus prompts. Cada edición se guarda, para que puedas comparar versiones y restaurar iteraciones anteriores con un clic.
</Callout>

Para prompts auto-gestionados, usa una estructura de carpetas:
...
Empieza simple, observa cuidadosamente, cambia una cosa a la vez, documenta lo que funciona y sabe cuándo parar. Los mejores prompts no se escriben—se descubren a través de iteración sistemática.
</Callout>
...
Intenta mejorar este prompt débil tú mismo. Edítalo, luego usa IA para comparar tu versión con la original:
...
Contexto:
- Me reuní con Sara Chen, VP de Marketing en TechCorp
- Discutimos nuestra plataforma de analytics
...
- Incluye un siguiente paso claro (agendar una demo)
- Mantén bajo 150 palabras

Formato: Línea de asunto + cuerpo del email`}
  task="Transforma este prompt de email vago en algo que produzca un resultado profesional y efectivo."
/>

En el próximo capítulo, exploraremos el prompting con JSON y YAML para aplicaciones de datos estructurados.
