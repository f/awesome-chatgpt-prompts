...
Los Modelos de Lenguaje Grande (LLMs) son sistemas de IA que aprendieron leyendo enormes cantidades de texto. Pueden escribir, responder preguntas y tener conversaciones que suenan humanas. Se llaman "grandes" porque tienen miles de millones de pequeños ajustes (llamados parámetros) que se ajustaron durante el entrenamiento.
...
Cuando escribes "La capital de Francia es...", la IA predice "París" porque eso es lo que usualmente viene después en textos sobre Francia. Esta idea simple, repetida miles de millones de veces con cantidades masivas de datos, crea un comportamiento sorprendentemente inteligente.

<TokenPredictionDemo />

### Conceptos Clave

**Tokens**: La IA no lee letra por letra. Divide el texto en fragmentos llamados "tokens". Un token puede ser una palabra completa como "hola" o parte de una palabra como "ción". Entender los tokens ayuda a explicar por qué la IA a veces comete errores de ortografía o tiene dificultades con ciertas palabras.

<Callout type="info" title="¿Qué es un Token?">
Un token es la unidad más pequeña de texto que un modelo de IA procesa. No siempre es una palabra completa—podría ser un fragmento de palabra, puntuación o espacio en blanco. Por ejemplo, "increíble" podría convertirse en 2 tokens: "increí" + "ble". En promedio, **1 token ≈ 4 caracteres** o **100 tokens ≈ 75 palabras**. Los costos de API y límites de contexto se miden en tokens.
</Callout>

<TokenizerDemo />

**Ventana de Contexto**: Esto es cuánto texto puede "recordar" la IA en una conversación. Piénsalo como la memoria a corto plazo de la IA. Incluye todo: tu pregunta Y la respuesta de la IA.
...
**Temperatura**: Esto controla cuán creativa o predecible es la IA. Temperatura baja (0.0-0.3) te da respuestas enfocadas y consistentes. Temperatura alta (0.7-1.0) te da respuestas más creativas y sorprendentes.

<TemperatureDemo />

**Prompt de Sistema**: Instrucciones especiales que le dicen a la IA cómo comportarse durante toda una conversación. Por ejemplo, "Eres un profesor amigable que explica las cosas de forma simple." No todas las herramientas de IA te permiten configurar esto, pero es muy poderoso cuando está disponible.

## Tipos de Modelos de IA

### Modelos de Texto (LLMs)
El tipo más común, estos generan respuestas de texto a entradas de texto. Impulsan chatbots, asistentes de escritura y generadores de código. Ejemplos: GPT-4, Claude, Llama, Mistral.

### Modelos Multimodales
Estos pueden entender más que solo texto. Pueden mirar imágenes, escuchar audio y ver videos. Ejemplos: GPT-4V, Gemini, Claude 3.

### Modelos de Texto a Imagen

<Callout type="info" title="Sobre Este Libro">
Aunque este libro se centra principalmente en prompting para Modelos de Lenguaje Grande (IA basada en texto), los principios de prompts claros y específicos también aplican a la generación de imágenes. Dominar los prompts para estos modelos es igualmente importante para obtener excelentes resultados.
</Callout>
...
<Compare 
  ...
  after={{ label: "Prompt Estilo Imagen", content: "gato atigrado naranja, sentado en alféizar, mirando la lluvia, interior acogedor, iluminación natural suave, fotorrealista, profundidad de campo, 4K" }}
/>
...
Los prompts de video necesitan describir acción a lo largo del tiempo, no solo una escena estática. Incluye verbos y movimiento:
</Callout>

<Compare 
  before={{ label: "Estático (Débil)", content: "Un pájaro en una rama" }}
  after={{ label: "Con Movimiento (Fuerte)", content: "Un pájaro alza el vuelo desde una rama, alas extendiéndose ampliamente, hojas moviéndose mientras despega" }}
/>
...
A veces la IA escribe cosas que suenan verdaderas pero no lo son. Esto se llama "alucinación". No es un error. Es simplemente cómo funciona la predicción. Siempre verifica los hechos importantes.
</Callout>

¿Por qué la IA inventa cosas?

1. Intenta escribir texto que suene bien, no texto que siempre sea verdadero
2. Internet (donde aprendió) también tiene errores
...
La IA no sabe las cosas por arte de magia. Pasa por tres pasos de aprendizaje, como ir a la escuela:
...
<Compare 
  before={{ label: "Antes del Fine-tuning", content: "Usuario: ¿Cuánto es 2+2?\nIA: 2+2=4, 3+3=6, 4+4=8, 5+5=10..." }}
  after={{ label: "Después del Fine-tuning", content: "Usuario: ¿Cuánto es 2+2?\nIA: 2+2 es igual a 4." }}
/>

### Paso 2: Fine-tuning (Aprender a Ayudar)
...
- "Cuando te pidan hacer algo dañino, rechaza educadamente"
...
Prueba el prompt de arriba. ¿Notas cómo la IA rechaza? Eso es el fine-tuning en acción.

### Paso 3: RLHF (Aprender lo que les Gusta a los Humanos)

RLHF significa "Aprendizaje por Refuerzo a partir de Retroalimentación Humana". Es una forma elegante de decir: los humanos califican las respuestas de la IA, y la IA aprende a dar mejores.

Así es como funciona:
1. La IA escribe dos respuestas diferentes a la misma pregunta
2. Un humano elige cuál respuesta es mejor
3. La IA aprende: "Okay, debería escribir más como la Respuesta A"
4. Esto pasa millones de veces

Por esto la IA:
- Es cortés y amigable
...
Conocer estos tres pasos te ayuda a entender el comportamiento de la IA. Cuando la IA rechaza una solicitud, eso es fine-tuning. Cuando la IA es extra cortés, eso es RLHF. Cuando la IA sabe hechos aleatorios, eso es pre-entrenamiento.
</Callout>
...
La IA no sabe nada sobre ti a menos que se lo digas. Cada conversación comienza de cero. Incluye la información de fondo que la IA necesita.

<Compare 
  before={{ label: "Falta Contexto", content: "¿Es un buen precio?" }}
...
<div className="my-4 grid gap-2">
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
    ...
    <span className="text-muted-foreground">Modelos rápidos como GPT-4o o Claude 3.5 Sonnet</span>
  </div>
  <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
...
<Quiz 
  ...
  options={[
    ...
    "Porque intenta escribir texto que suena bien, no texto que siempre sea verdadero",
    "Porque no tiene suficientes datos de entrenamiento",
    "Porque la gente escribe malos prompts"
  ]}
  correctIndex={1}
  explanation="La IA está entrenada para predecir lo que suena correcto, no para verificar hechos. No puede buscar cosas o verificar si algo es verdad, así que a veces escribe con confianza cosas que son incorrectas."
/>
...
<TryIt 
  ...
  description="Pídele a la IA que se explique. Ve cómo habla sobre ser un modelo de predicción y admite sus límites."
/>
