...
<Callout type="tip" title="相關內容：角色化提示">
系統提示詞建立在[角色化提示](/book/04-role-based-prompting)的概念之上。角色提示在你的消息中分配一個角色，而系統提示詞則在更深層次上設定這個身份，並在整個對話過程中保持不變。
</Callout>
...
<InfoGrid items={[
  { label: "1. 系統消息（隱藏）", description: "\"你是一個友好的烹飪助手，專注於快速的工作日晚餐...\"", color: "purple" },
  { label: "2. 使用者消息（你的問題）", description: "\"用雞肉和米飯可以做什麼？\"", color: "blue" },
  { label: "3. 助手消息（AI回覆）", description: "\"這裡有一道20分鐘雞肉炒飯，非常適合忙碌的夜晚！...\"", color: "green" },
]} />
...
<Checklist 
  ...
  items={[
    { text: "基本任務：它能做到你想要的嗎？" },
    { text: "邊緣情況：面對不尋常的請求會發生什麼？" },
    { text: "邊界：它能保持在限制範圍內嗎？" },
    { text: "個性：它能保持角色一致嗎？" },
    { text: "長對話：在很多消息之後它還能保持一致嗎？" },
...
<TryIt 
  title="建立你自己的"
  description="使用這個模板建立你自己的系統提示詞。填寫空白處！"
