...
<InfoGrid items={[
  ...
  { label: "速度 vs. 品質", description: "更快的模型可能會犧牲一些能力", example: "GPT-4 比 GPT-4o-mini 更強但更慢", exampleType: "text", color: "purple" },
  { label: "一致性 vs. 創造性", description: "較低的 temperature = 更可預測但創造性更低", example: "事實性內容用 0.2，頭腦風暴用 0.8", exampleType: "text", color: "green" },
  { label: "簡單性 vs. 穩健性", description: "邊緣情況處理會增加複雜性", example: "簡單的提示詞在異常輸入時會失敗", exampleType: "text", color: "amber" }
]} />
...
<InfoGrid items={[
  ...
  { label: "相關性", description: "是否回答了實際被問到的問題？", example: "回覆直接回答問題 vs. 跑題", exampleType: "text", color: "blue" },
  { label: "完整性", description: "是否涵蓋了所有要求？", example: "輸出中包含所有 5 個要求的部分", exampleType: "text", color: "blue" },
  { label: "延遲", description: "回應到達需要多長時間？", example: "聊天應用 p50 < 2秒，p95 < 5秒", exampleType: "text", color: "purple" },
...
<TryIt 
  title="定義你的成功指標"
  description="在做出更改之前，使用此模板來明確你要優化的目標。"
...
<InfoGrid items={[
  ...
  { label: "透過位置引用", description: "指向內容而不是重複它", example: "\"the text above\" 而不是重新引用", color: "green" }
]} />
...
<InfoGrid items={[
  ...
  { label: "請求置信度", description: "讓不確定性明確化", example: "\"Rate your confidence 1-10 and explain any uncertainty\"", color: "blue" },
...
<InfoGrid items={[
  { label: "詳細格式規範", description: "準確展示輸出應該是什麼樣子", example: "包含模板或 schema", exampleType: "text", color: "purple" },
...
<InfoGrid items={[
  ...
  { label: "互動式（< 2s）", description: "快速模型，啟用流式輸出", example: "GPT-4o-mini 配合流式輸出", exampleType: "text", color: "amber" },
  { label: "可容忍（< 10s）", description: "中等模型，平衡品質/速度", example: "GPT-4o、Claude Sonnet", exampleType: "text", color: "green" },
  { label: "異步/批量", description: "使用最佳模型，後臺處理", example: "GPT-4、Claude Opus 用於離線處理", exampleType: "text", color: "blue" }
]} />
...
<InfoGrid items={[
  ...
  { label: "使用流式輸出", description: "更快獲得第一個 token，更好的使用者體驗", example: "任何超過 100 tokens 的回應都使用流式", exampleType: "text", color: "cyan" },
  { label: "積極快取", description: "不要重複計算相同的查詢", example: "快取常見問題、模板輸出", exampleType: "text", color: "cyan" }
]} />
...
<InfoGrid items={[
  ...
  { label: "批次處理", description: "將相關查詢合併到單一請求中", example: "在一個提示詞中分析 10 筆資料 vs. 10 次單獨呼叫", exampleType: "text", color: "green" },
  { label: "預過濾", description: "不要傳送不需要 AI 的請求", example: "在昂貴的分類之前使用關鍵詞匹配", exampleType: "text", color: "green" }
]} />
...
<InfoGrid items={[
  { label: "提示詞文件", description: "儲存精確的提示詞文字，包括系統提示詞和任何模板", example: "像管理程式碼一樣對提示詞進行版本控制", exampleType: "text", color: "blue" },
  ...
  { label: "效能指標", description: "測量每個測試用例的 token 和時間", example: "平均輸入：450 tokens，平均輸出：200 tokens，p50 延遲：1.2s", exampleType: "text", color: "purple" }
]} />

<TryIt 
  title="基準文件模板"
  description="在優化之前使用此模板建立全面的基準。"
...
<Quiz 
  ...
  correctIndex={2}
  explanation="在優化之前，先測量。你需要了解 token 花在哪裡，然後才能有效地減少它們。提示詞可能有不必要的上下文、冗長的指令，或產生比需要的更長的輸出。測量告訴你應該把優化工作集中在哪裡。"
/>
