...
列表非常適合分步指令、排名項目或相關要點的集合。它們易於瀏覽和解析。當順序重要時使用**編號列表**（步驟、排名），對於無序集合使用**項目符號**。
...
<TryIt 
  title="JSON 提取"
  description="從非結構化文字中提取結構化資料。"
  prompt={`從這段文字中提取資訊並以 JSON 格式返回：
...
模型有時會在 JSON 周圍新增解釋性文字或 markdown 格式。透過關於輸出格式的明確指令來防止這種情況。你可以請求原始 JSON 或程式碼塊中的 JSON——根據你的解析需求選擇。
...
<TryIt 
  compact
  title="YAML 產生"
  prompt={`為 Node.js 專案產生 GitHub Actions 工作流程。
...
<Quiz 
  question="結構化輸出相對於非結構化文字的主要優勢是什麼？"
  options={[
    "它使用更少的 token",
    "AI 更容易產生",
    "可以程式化解析和驗證",
  ...
  correctIndex={2}
  explanation="像 JSON 這樣的結構化輸出可以被程式碼解析、跨查詢比較、整合到工作流程中並驗證完整性——這些對於自由格式文字來說是困難或不可能的。"
/>
