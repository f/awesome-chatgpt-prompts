...
<Compare 
  before={{ label: "❌ 模糊的請求", content: "寫一個驗證信箱的函式。" }}
  after={{ label: "✓ 完整的規格說明", content: "寫一個 Python 函式來驗證信箱地址。\n\n輸入：string（可能的信箱地址）\n輸出：tuple[bool, str | None] - (is_valid, error_message)\n處理：空字串、None、unicode 字元\n使用正則表達式，包含類型註解和說明字串。" }}
/>
...
<Callout type="tip" title="除錯原則">
始終包含**預期行為**、**實際行為**和**錯誤資訊**（如有）。你提供的上下文越多，AI 就能越快找到根本原因。
</Callout>
...
```
我希望你扮演一位程式碼審查員。我會提供包含程式碼變更的
Pull Request，你將對其進行全面審查。檢查 bug、安全問題、
效能問題以及是否遵循最佳實踐。提供建設性的反饋，
幫助開發者改進。
```
...
包含完整的上下文（語言、框架、約束條件），精確說明需求，請求特定的輸出格式，在請求程式碼的同時要求解釋，幷包含需要處理的邊界情況。
</Callout>

<Quiz 
  question="請求 AI 除錯程式碼時，最重要的元素是什麼？"
  options={[
    "僅提供程式設計語言",
    "預期行為、實際行為和錯誤資訊",
    "僅提供程式碼片段",
  ...
  correctIndex={1}
  explanation="除錯需要上下文：應該發生什麼與實際發生了什麼。錯誤資訊和堆棧跟蹤能幫助 AI 快速定位確切問題。"
/>
